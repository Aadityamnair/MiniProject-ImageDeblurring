...
# constructing the argument parser for defining epochs
parser = argparse.ArgumentParser()
parser.add_argument('-e', '--epochs', type=int, default=40, 
            help='number of epochs to train the model for')
#args = vars(parser.parse_args())
args, unknown = parser.parse_known_args()
def save_decoded_image(img, name): #to save deblured image -called during validation
...
#getting hold of image file paths
gauss_blur = os.listdir('input/gaussian_blurred')
gauss_blur.sort() #to align both paths
sharp = os.listdir('input/sharp')
sharp.sort()
x_blur = [] #training data 
...
class DeblurCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.conv2(x))#rectified linear activation unit-does not activate all neurons ata time
        x = self.conv3(x)
        return x
model = DeblurCNN().to(device)
print(model)

# the loss function #distance between current output and expected output
criterion = nn.MSELoss()#compare pixels of blured image and sharpe image
# the optimizer - modifies attributes of neural network(weight,lr)
optimizer = optim.Adam(model.parameters(), lr=0.001)#lr-weights updated during training #adam optimizer-handle sparse gradient in noisy pblms
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau( 
        optimizer,
        mode='min',
        patience=5,#no of epochs with no improvement aftere which training will be stopped
        factor=0.5,
        verbose=True #how you want to see your output while training
    )
...
def fit(model, dataloader, epoch):
    ...
    for i, data in tqdm(enumerate(dataloader), total=int(len(train_data)/dataloader.batch_size)):
        ...
        optimizer.zero_grad()#set the gradient
